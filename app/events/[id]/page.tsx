...
export default async function EventDetailPage({ params }: { params: { id: string } }) {
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6">
          ...
          <Link href="/planner" className="text-blue-600 hover:underline mt-4 inline-block">
            ← Terug naar planner
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <Link href="/planner" className="text-blue-600 hover:underline mb-4 inline-block">
        ← Terug naar planner
      </Link>
      ...
      <div className="bg-white rounded-lg shadow-md p-8 mt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              ...
              {event.category && (
                <div>
                  ...
                  <p className="text-gray-800 capitalize">{event.category}</p>
                </div>
              )}
              ...
              {event.audience && (
                <div>
                  ...
                  <p className="text-gray-800">{event.audience}</p>
                </div>
              )}

              <div>
                <span className="text-sm font-medium text-gray-600">Schooljaar:</span>
                <p className="text-gray-800">{event.school_year}</p>
              </div>
          ...
          <div>
            <h2 className="text-lg font-semibold text-gray-700 mb-4">Beschrijving</h2>
            ...
            {event.approver && (
              <div className="mt-3">
                <span className="text-sm font-medium text-gray-600">Goedgekeurd door:</span>
                <p className="text-gray-800">{event.approver.name}</p>
              </div>
            )}
          </div>
        </div>
